...
export function getShopName() {
  ...
  const domain = url.parse(Core.Reaction.absoluteUrl()).hostname;
...
export function getAbsoluteUrl(path) {
  ...
  if (prefix) {
    const absUrl = Core.Reaction.absoluteUrl(`${prefix}/${path}`);
    return absUrl;
  }
  const absUrl = Core.Reaction.absoluteUrl(`${path}`);
  return absUrl;
}
