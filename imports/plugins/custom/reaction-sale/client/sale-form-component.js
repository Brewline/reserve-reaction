...
function dateTimeIsoStringWithZone(date, time) {
  if (!date || !time) { return ""; }
...
export default class SaleFormComponent extends Component {
  ...
  onSubmit = (event) => {
    ...
    if (beginsAt) {
      ...
      formData.beginsAt = new Date(Date.parse(beginsAt));
    }

    if (endsAt) {
      // formData.endsAt = convertToUtcDate(endsAt);
      formData.endsAt = new Date(Date.parse(endsAt));
    }

    const { changedSlugWas, slug: previousSlug } = sale;
    if (fields.slug !== previousSlug) {
      fields.changedSlugWas = previousSlug;
    } else {
      fields.changedSlugWas = changedSlugWas;
    }
